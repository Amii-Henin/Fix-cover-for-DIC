...
# 音乐库账号设置
mb.set_useragent('name', '1.0', 'example@mail.com')
mb.set_rate_limit(2, 1)
mb.set_hostname("musicbrainz.org", use_https=True)
mb.set_caa_hostname("coverartarchive.org", use_https=True)
dz = deezer.Client()
...
matched_handler = logging.FileHandler(
    "./log/dic_recover_matched.log", encoding="utf-8")
...
miss_handler = logging.FileHandler(
    "./log/dic_recover_missed.log", encoding="utf-8")
...
chk_handler = logging.FileHandler(
    "./log/dic_recover_chk.log", encoding="utf-8")
...
update_handler = logging.FileHandler(
    "./log/dic_recover_updated.log", encoding="utf-8")
...
upload_handler = logging.FileHandler(
    "./log/dic_recover_upload.log", encoding="utf-8")
...
cmp_handler = logging.FileHandler(
    "./log/dic_recover_cmp.log", encoding="utf-8")
...
tmp_handler = logging.FileHandler(
    "./log/dic_recover_tmp.log", encoding="utf-8")
tmp_handler.setFormatter(logging.Formatter(
    "%(asctime)s - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S"
))
tmpLog.addHandler(tmp_handler)
# 阻止传播到父logger
...
def flush_batch(conn, cursor, batch_full, batch_update, batch_simple, pip_list, ppp_str):
    ...
    try:
        # 跳过空集
        if (len(batch_full) + len(batch_simple) + len(batch_update)) < 1:
            return False
        # 批量写入数据库
        if batch_simple:
        ...
        for f in batch_full:
            all.append(f[0])
        for f in batch_update:
            all.append(f[0])
        for f in batch_simple:
            all.append(f[0])
        all.sort()
...
def get_all_group(newStartNum):
    ...
    cursor.execute(
        f'SELECT groupID FROM {TABLE} ORDER BY groupID DESC LIMIT 1')
    data = cursor.fetchone()
    maxIndex = data[0] if data else 1
    if newStartNum:
        start_id = newStartNum
    else:
        start_id = maxIndex + 1
    for i in range(10):
        logging.info('')
    ...
    logging.info(f'{"#"*80}')
    matchLog.info(f'{"#"*80}')
    matchLog.info(f'{"#"*35} 开始运行 {"#"*35}')
    matchLog.info(f'{"#"*80}')
    missMatchLog.info(f'{"#"*80}')
    missMatchLog.info(f'{"#"*35} 开始运行 {"#"*35}')
    missMatchLog.info(f'{"#"*80}')

    for groupId in range(start_id, ENDID + 1):
        try:
            # 先从数据库中提取，cover是pipimg再获取json数据判断
            if groupId <= maxIndex:
                cursor.execute(
                    f'SELECT groupID,json,cover,newCover FROM {TABLE} WHERE groupID={groupId} LIMIT 1')
                data = cursor.fetchone()
                if not data[2] or not 'ptpimg' in data[2] or (data[3] and len(data[3]) > 10):
                    continue
            ...
            if res.status_code == 200:
                if "DOCTYPE" in res.text[:10]:
                    ...
                    # 正常JSON数据，提取cover
                    cover = json.loads(res.text)[
                        "response"]["group"]["wikiImage"]
                    ...
                    if 'ptpimg' in cover:
                        # count += 1
                        # 替换cover
                        newCover, urls = upload_cover(
                            groupId, json.loads(res.text), count)
                        if newCover:
                            update_desc(groupId, newCover)
                            pip_list.append(f'{DIC}{groupId}    {newCover}')
                        else:
                            pip_list.append(f'{DIC}{groupId}')
                        # ppp_str += f'{groupId} '
                        if urls:
                            ...
                            urls = None
                        batch_full.append(
                            (groupId, res.text, cover, newCover, urls))
                    else:
                        batch_full.append(
                            (groupId, res.text, cover, None, urls))
            else:
                batch_simple.append((groupId, f"status_{res.status_code}"))
                logging.error(
                    f'{groupId} 响应 {res.status_code} - {res.text[:20]}')
        ...
        # 达到批次大小，统一刷写
        if count >= BATCH_SIZE:
            flush_batch(conn, cursor, batch_full, batch_update,
                        batch_simple, pip_list, ppp_str)
            # 清空批次缓存
            batch_full.clear()
            batch_update.clear()
            batch_simple.clear()
...
# 寻找合适的封面并上传
def upload_cover(id, jsonData, count):
    # title, album, artist, year, artists, urls, desc
    musicInfo = album_info(jsonData)
    errList = []
    # 处理json数据失败，返回none
    if musicInfo[0] == None:
        return (None, None)
    ...
    for url in urls:
        # 跳过
        if 'musicbrainz' in url and SKIPMB:
            continue
        alb, art, arts, yr, trk, img = get_info_from_url(url)
        if not alb:
            continue
        score = calc_match_score(album, artist, ' '.join(
            artists), year, desc, alb, art, arts, yr, trk, id)
        logging.info(f'【{score}】 {art} - {alb} ({yr})')
        # 匹配度达标
        if score > THRESHOLD_URL:
            logging.info(f'■■■■■■【匹配成功】■■■■■■ {DIC}{id}')
            matchLog.info(f'{"="*8} {id} 【{score}】 {"="*8}')
    ...
    if len(errList):
        ...
        if min(tmp) < THRESHOLD_CHK:
            missMatchLog.info(f'{"="*8} {id} {"="*8}')
            missMatchLog.info(f'{title} ({year}) - {DIC}{id}')
        if max(tmp) >= THRESHOLD_CHK:
            chkLog.info(f'{"="*8} {id} {"="*8}')
            chkLog.info(f' {title} ({year}) - {DIC}{id}')
        for data in errList:
            if data[0] < THRESHOLD_CHK:
                missMatchLog.info(
                    f'【{data[0]}】 {data[2]} - {data[1]} ({data[3]}) - {data[4]} {id}')
            else:
                chkLog.info(
                    f'【{data[0]}】 {data[2]} - {data[1]} ({data[3]}) - {data[4]} {id}')
    # 从iTunes，discogs，musicbrainz，Deezer匹配匹配
    return (match_cover(id, musicInfo), urls)

# 从iTunes，Discogs，Musicbrainz, Deezer匹配


def match_cover(id, musicInfo):
    try:
        title, album, artist, year, artists, urls, desc = musicInfo
        allData = [['iTunes'], ['Discogs'], ['Deezer'], ['Brainz']]
        allScore = [0, 0, 0, 0]
        u = 0
        ###############
        #   iTunes    #
        ###############
        try:
            url = f"https://itunes.apple.com/search?term={title}&media=music&entity=album&limit=1"
            response = requests.get(url, headers=HEADITUNES)
            response.raise_for_status()
            data = response.json()
        except Exception as e:
            logging.warning(f'iTunes 获取数据出错: {e}')
            data = {'results': []}
        if len(data['results']) < 1:
            scoreIT = 0
            allData[0].append((None, None, None, None, None, None))
            allData[0].append(None)
            logging.info(f'[iTunes]【0】')
        else:
            albumUrl = data['results'][0]['collectionViewUrl']
            allData[0].append(get_info_from_url(albumUrl))
            allData[0].append(albumUrl)
            alb, art, arts, yr, trk, img = allData[0][1]
            if not alb:
                scoreIT = 0
            else:
                u += 1
                scoreIT = calc_match_score(album, artist, ' '.join(
                    artists), year, desc, alb, art, arts, yr, trk, id)
            allData[0].append(scoreIT)
            allScore[0] = scoreIT
            logging.info(f'[iTunes]【{scoreIT}】 {art} - {alb} ({yr})')
        ###############
        #   Discogs   #
        ###############
        try:
            abs = ds.search(album, artist=artist, type='release')
        except Exception as e:
            logging.warning(f'Discogs 获取数据出错: {e}')
            abs = []
        if len(abs) < 1:
            scoreDS = 0
            allData[1].append((None, None, None, None, None, None))
            allData[1].append(None)
            logging.info(f'[Discogs]【0】')
        else:
            albumUrl = None
            for a in abs:
                tmp = a.data
                if 'type' in tmp and tmp['type'] == 'release':
                    albumUrl = tmp['resource_url'].replace(
                        'api', 'www').replace('releases', 'release')
                    break
            allData[1].append(get_info_from_url(albumUrl))
            allData[1].append(albumUrl)
            alb, art, arts, yr, trk, img = allData[1][1]
            if not alb:
                scoreDS = 0
            else:
                u += 1
                scoreDS = calc_match_score(album, artist, ' '.join(
                    artists), year, desc, alb, art, arts, yr, trk, id)
            allData[1].append(scoreDS)
            allScore[1] = scoreDS
            logging.info(f'[Discogs]【{scoreDS}】 {art} - {alb} ({yr})')
        ###############
        #   Deezer    #
        ###############
        try:
            abs = dz.search_albums(title)
        except Exception as e:
            logging.warning(f'Deezer 获取数据出错: {e}')
            abs = ''
        if len(abs) < 1:
            scoreDZ = 0
            allData[2].append((None, None, None, None, None, None))
            allData[2].append(None)
            logging.info(f'[Deezer]【0】')
        else:
            albumUrl = f"https://www.deezer.com/album/{abs[0].id}"
            allData[2].append(get_info_from_url(albumUrl))
            allData[2].append(albumUrl)
            alb, art, arts, yr, trk, img = allData[2][1]
            if not alb:
                scoreDZ = 0
            else:
                u += 1
                scoreDZ = calc_match_score(album, artist, ' '.join(
                    artists), year, desc, alb, art, arts, yr, trk, id)
            allData[2].append(scoreDZ)
            allScore[2] = scoreDZ
            logging.info(f'[Deezer]【{scoreDZ}】 {art} - {alb} ({yr})')
        ###############
        # MusicBrainz #
        ###############
        if max(allScore) < THRESHOLD_SEARCH and not SKIPMB:
            try:
                abs = mb.search_releases(album, artist=artist, limit=2)
            except Exception as e:
                logging.warning(f'Brainz 获取数据出错: {e}')
                abs = {'release-list': ''}
            if len(abs['release-list']) < 1:
                scoreMB = 0
                allData[3].append((None, None, None, None, None, None))
                allData[3].append(None)
                logging.info(f'[Brainz]【0】')
            else:
                time.sleep(1.3 + random.random() * 2)
                albumUrl = f"https://musicbrainz.org/release/{abs['release-list'][0]['id']}"
                allData[3].append(get_info_from_url(albumUrl))
                allData[3].append(albumUrl)
                alb, art, arts, yr, trk, img = allData[3][1]
                if not alb:
                    scoreMB = 0
                else:
                    u += 1
                    scoreMB = calc_match_score(album, artist, ' '.join(
                        artists), year, desc, alb, art, arts, yr, trk, id)
                allData[3].append(scoreMB)
                allScore[3] = scoreMB
                logging.info(f'[Brainz]【{scoreMB}】 {art} - {alb} ({yr})')
        ########################
        # 对比取匹配对最高的数据 #
        ########################
        maxScore = max(allScore)
        idx = allScore.index(maxScore)
        alb, art, arts, yr, trk, img = allData[idx][1]
        if maxScore > THRESHOLD_SEARCH:
            logging.info(f'■■■■■■【匹配成功】■■■■■■ {DIC}{id}')
            matchLog.info(
                f'{"="*8} {id} 【{maxScore}】 [{allData[idx][0]}] {"="*8}')
            matchLog.info(f'{title} ({year}) - {DIC}{id}')
            matchLog.info(f'{art} - {alb} ({yr}) - {allData[idx][2]}')
            return upload_to_dic(id, img, f'{id}_cover.jpg')
        else:
            logging.info(f'◀◀◀◀◀◀ 匹配失败 ▶▶▶▶▶▶')
            if maxScore > 10:
                if maxScore < 40:
                    ...
                    missMatchLog.info(f'{title} ({year}) - {DIC}{id}')
                    missMatchLog.info(
                        f'{art} - {alb} ({yr}) - {allData[idx][2]} {id}')
                else:
                    scores = allScore.copy()
                    ...
                    for score in scores:
                        if score > 0:
                            idx = allScore.index(score)
                            if u > 1:
                                chkLog.info(
                                    f'[{allData[idx][0]} {int(score)}] {allData[idx][1][1]} - {allData[idx][1][0]} ({allData[idx][1][3]}) - {allData[idx][2]} {id}')
                            else:
                                chkLog.info(
                                    f'{allData[idx][1][1]} - {allData[idx][1][0]} ({allData[idx][1][3]}) - {allData[idx][2]} {id}')
                    # chkLog.info(f'')
                    # chkLog.info(f'')
            return None
    except Exception as e:
        logging.error(f'{id} 匹配 {title} 出错 - {e}')
        return None
...
def get_info_from_url(url):
    ...
    if 'musicbrainz' in url:
        if not 'release' in url:
            logging.info(f'url非release: {url}')
        else:
            return mb_release(url, True)
    elif 'apple.com' in url:
        return itunes_info_url(url, True)
    elif 'discogs' in url:
        if not '/release' in url and not '/master' in url:
            logging.info(f'非专辑url: {url}')
        else:
            return discogs_get_url(url, True)
    elif 'deezer' in url:
        if not '/album/' in url:
            logging.info(f'非专辑url: {url}')
        else:
            return deezer_get_url(url, True)
    else:
...
def album_info(jsonData):
    try:
        # 跳过无数据
        if jsonData == 'not exist' or jsonData == 'error':
            return None
        # 获取元数据
        ...
        if len(artists):
            if len(artists) > 2:
                ...
                artist = f'{html.unescape(artists[0]["name"])} & {html.unescape(artists[1]["name"])}'
            else:
        ...
        for typ in musicInfo.keys():
            for art in musicInfo[typ]:
                try:
                    artists.append(html.unescape(art['name']))
                except:
                    logging.error(f'{all["id"]} 解析 {typ} 失败 - {art}')
        ...
        # url去重，删除非音乐库的链接
        urls = list(set(urls))
        musicUrls = ['apple.com', 'discogs.com', 'musicbrainz.org', 'bandcamp.com', 'deezer.com', 'qobuz.com', 'bugs.co', 'amazon.co', 'allmusic.com', 'spotify.com', 'kkbox.com', '163.com', 'tidal.com', 'universal-music.co',
                     'bleep.com', 'vgmdb.net', 'vocadb.net', 'oricon.co', 'prestomusic.com', 'beatport.com', 'highresaudio.com', 'melon.com', 'metal-archives.com', 'nme.com', 'cdjapan.co', 'soundcloud.com', 'ototoy.jp', 'pitchfork.com']
        tmp = []
        # 仅保留部分url
        for u in urls:
            for i in musicUrls:
...
def itunes_info_url(url, get_pic=False):
    ...
    if len(album_id):
        album_id = re.search('\d+', album_id).group()
    ...
    try:
        ...
        # 提取信息
        info = {
            'album': album_data.get('collectionName').replace(' - Single', '').replace(' - EP', ''),
            'artist': album_data.get('artistName'),
            'artists': [],
            'date': int(album_data.get('releaseDate', '')[:4]),
            # 获取更高清的封面
            'cover': album_data.get('artworkUrl100', '').replace('100x100', '1200x1200'),
            'tracks': ''
        }
...
def mb_release(url, get_img=False):
    try:
        ...
        res = mb.get_release_by_id(
            id, includes=["artists", "recordings", "media"])
        album = res['release']['title']
        artist = res['release']['artist-credit'][0]['artist']['name']
        artists = []
        if 'date' in res['release']:
            year = res['release']['date']
        else:
            year = ''
        if len(year) > 5:
            year = int(year[:4])
...
def calc_match_score(
    ...
    sim_artist = cals_multi(input_artist, item_artist,
                            id, input_album, input_artist)
    score_components.append((sim_artist, w))
    active_weight_sum += w

    # 专辑名
    w = RAW_WEIGHTS["album"]
    sim_album = cals_multi(input_album, item_album, id,
                           input_album, input_artist)
    ...
    if input_tracks is not None and input_tracks.strip():
        sim_first = fuzz.token_sort_ratio(
            input_tracks, item_tracks) if item_tracks else 0
...
def cals_multi(org, chk, id, album, artist):
    ...
    cmpLog.info(
        f'[all {allScore[0]}] [us: {type["us"]}, cn: {type["cn"]}, kr: {type["kr"]}]')
    # 对比单种语言
    for cty in type.keys():
        if type[cty] > 0:
            corg = keep_cty(org, cty)
            cchk = keep_cty(chk, cty)
            if len(corg) and len(cchk):
                score = fuzz.token_sort_ratio(corg, cchk)
            else:
                score = 0
...
def image_bin_to_jpg(
    ...
    try:
        inSize = int(len(img_bytes) / 1024)
        input_stream = io.BytesIO(img_bytes)
        if inSize < avg and quality < 95:
            quality = 95
        with Image.open(input_stream) as im:
            ...
            if max_side != 0:
                if inSize > avg and (width > max_side or height > max_side):
                    # 计算缩放比例
            ...
            out_img.save(output_buf, format="JPEG",
            ...
            if max_size_kb is not None:
                ...
                outSize = int(len(jpg_bin) / 1024)
                if len(jpg_bin) > max_bytes:
                    # 首先检查最低质量（0）是否仍超标
                    output_buf = io.BytesIO()
                    out_img.save(output_buf, format="JPEG",
                                 quality=0, optimize=True)
                    ...
                    jpg_bin = output_buf.getvalue()
                    logging.info(
                        f'{id} 图片大小: {inSize} -> {outSize} -> {int(len(jpg_bin) / 1024)}')
        outSize = int(len(jpg_bin) / 1024)
        tmpLog.info(f'{id}: {inSize} -> {outSize}')
...
def upload_to_dic(id, img, imgName):
    ...
    try:
        img = image_bin_to_jpg(img, id=id)
        if img[0]:
            files = {"image": (imgName, img[1], "image/jpeg")}
        res = requests.post(url, files=files, headers=head)
        result = json.loads(res.content)
        if result['status'] == 'success':
            uploadLog.info(
                f'{id} 上传图片成功: {result["url"]}    size: {int(len(img[1]) / 1024)}')
            return result['url']
        else:
...
def update_desc(id, coverUrl):
    ...
    try:
        ...
        res = requests.get(url, headers=HEAD)
        info = json.loads(res.text)["response"]['group']
        if info['wikiImage'] != coverUrl:
            raise ValueError(f'提交表单失败：{info["wikiImage"]} -> {coverUrl}')
...
if __name__ == "__main__":
    newStartNum = input('初始ID（默认数据库最大值）：') or 0  # 14204
    newStartNum = int(newStartNum)
    ENDID = input(f'结束ID（默认{ENDID}）：') or ENDID
    ENDID = int(ENDID)
    BATCH_SIZE = input(f'批处理间隔（默认{BATCH_SIZE}）：') or BATCH_SIZE
    BATCH_SIZE = int(BATCH_SIZE)
    SKIPMB = input(f'跳过 MusicBrainz（默认否）：') or False
    get_all_group(newStartNum)
